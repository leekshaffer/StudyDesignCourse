...
For any random variables (RVs) $X$ and $Y$, constants $a$ and $b$, and events $C_1,\ldots,C_n$ that partition the sample space:

\begin{align*}
    E[X] &= P(X=1) \mbox{ for a binary RV } X \\
    E[X] &= \sum_{x \in supp(X)} x P(X=x) \mbox{ for a discrete RV } X \\
    E[X] &= \int_{-\infty}^{\infty} x f_X(x) \mbox{ for a continuous RV } X \mbox{ with PDF } f_X(x) \\
    E[aX + bY] &= a E[X] + b E[Y] \qquad \mbox{(linearity of expectations)} \\
    E[X] &= \sum_{i=1}^n E[X|C_i] P(C_i) \qquad \mbox{(law of total expectation)} \\
    P[X=1] &= \sum_{i=1}^n P[X=1|C_i] P(C_i) \qquad \mbox{(law of total probability)} \\
    E[X] &= E[E[X|Y]] \qquad \mbox{(law of total expectation or Adam's Law)} \\
\end{align*}
...
\begin{align*}
    ...
    Var(X+Y) &= Var(X-Y) = Var(X) + Var(Y) \mbox{ if } X \mbox{ and } Y \mbox{ are independent} \\
    Var(aX+bY) &= a^2 Var(X) + b^2 Var(Y) \mbox{ if } X \mbox{ and } Y \mbox{ are independent} \\
    Var(X) &= E[Var(X|Y)] + Var(E[X|Y]) \qquad \mbox{(law of total variance or Eve's Law)}
\end{align*}

### 1.3. Limiting Theorems

-   *i.i.d.:* A set of random variables are independent and identically distributed if they are all mutually independent and follow the same distribution.
...
-   *Central Limit Theorem (CLT):* Let $X_1,X_2,\ldots$ be i.i.d. random variables with mean $\mu$ and variance $\sigma^2$. Then as $n \to \infty$: $$ \sqrt{n} \left( \frac{\bar{X}_n - \mu}{\sigma} \right) \overset{\mathcal{D}}{\to} N(0,1). $$
...
*Properties:* Let $Y \sim Ber(p)$ and $S \sim Bin(n,p)$. Then: \begin{align*}
    E[Y] &= p \\
    E[1-Y] &= 1-p \\
    Var(Y) &= p(1-p) \\
    E[S] &= np \\
...
\begin{align*}
    ...
    P(\lvert X-\mu \rvert < 2\sigma) &\approx 0.95  \quad \mbox{(more accurately, } 1.96\sigma \mbox{)}\\
...
-   In other cases, the Poisson distribution where zeroes are removed (truncated) is a better model. We call this the zero-truncated Poisson distribution. If $Z \sim Pois(\lambda)$, then the zero-truncated version of $Z$, $Z|Z > 0$ has the following properties: \begin{align*}
...
*Properties:* Let $Z \sim Exp(\lambda)$. Then: \begin{align*}
    E[Z] &= 1/\lambda. \\
    Var(Z) &= 1/\lambda^2. \\
    F_Z(z) &= P[Z \le z] = 1-e^{-\lambda z} \mbox{, for } z > 0 \mbox{ (CDF)}. \\
    P[Z \ge s + t | Z \ge t] &= P[Z \ge s] \mbox{ for any } s,t >0 \mbox{ (memoryless)}.
\end{align*}
...
\begin{align*}
    ...
    Var(\bar{Y}_n) &= \frac{Var(Y_1)}{n} = \frac{\sigma^2_Y}{n} \quad \mbox{assuming independence of individuals}
\end{align*}
...
For 80% power, $Phi^{-1}(\Delta) = -0.842$; for 90% power, $Phi^{-1}(\Delta) = -1.282$. For $\alpha = 0.1$, $\Phi^{-1}(1-\alpha/2) = 1.65$; for $\alpha = 0.05$, $\Phi^{-1}(1-\alpha/2) = 1.960$; and for $\alpha = 0.01$, $\Phi^{-1}(1-\alpha/2) = 2.576$.
